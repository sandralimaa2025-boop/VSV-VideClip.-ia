...
export const GenerationView: React.FC<GenerationViewProps> = ({
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-6 space-y-8 animate-fadeIn">
        ...
        {scenes.map((scene, index) => {
          ...
          return (
            ...
            >
              ...
              <div
                onClick={() => isReady && setPreviewScene(scene)}
                ...
                {isReady && scene.generatedAssetUrl ? (
                  <>
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 pointer-events-none">
                      <span className="px-3 py-1 rounded-lg bg-black/70 border border-white/20 text-xs font-bold text-white flex items-center gap-1.5 backdrop-blur-sm shadow-xl">
      ...
      {/* Scene Full Preview Modal */}
      {previewScene && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#09090d] border border-violet-500/50 rounded-3xl max-w-4xl w-full overflow-hidden space-y-4 glow-purple animate-fadeIn">
            <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-1 rounded-md bg-violet-950/80 border border-violet-500/40 text-violet-300 font-mono text-xs font-bold">
                  CENA {previewScene.order.toString().padStart(2, '0')}
                </span>
                <h3 className="font-heading font-bold text-white text-sm">
                  {previewScene.musicSection} • {previewScene.duration} segundos
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className="relative aspect-video w-full bg-black flex items-center justify-center overflow-hidden">
              <img
                src={previewScene.generatedAssetUrl || previewScene.thumbnailUrl}
                alt={`Cena ${previewScene.order}`}
                referrerPolicy="no-referrer"
                className="w-full h-full object-cover"
              />
              {previewScene.lyricsSnippet && (
            ...
            <div className="p-4 border-t border-zinc-800/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-zinc-300">
              <div className="space-y-1">
                <p className="font-medium text-violet-300">🎯 Intenção: {previewScene.emotionalGoal}</p>
                <p className="text-zinc-400">📹 Câmera: {previewScene.cameraMovement} | Lente: {previewScene.lens || '35mm'}</p>
              </div>
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  ...
                  onClick={() => {
                    ...
                    setSelectedSceneForRegen(sc);
                  }}
                  className="px-4 py-2 rounded-xl bg-violet-950 hover:bg-violet-900 border border-violet-500/40 text-violet-300 hover:text-white font-bold flex items-center gap-2"
                >
                ...
                <button
                  type="button"
                  onClick={() => setPreviewScene(null)}
                  className="px-4 py-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-white font-bold"
                >
                  Fechar
                </button>
              </div>
